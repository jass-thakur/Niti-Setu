  ...
  return (
    <>
                ...
                {isSearchFocused && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border shadow-2xl rounded-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200 min-w-[320px]">
                    {searchQuery.trim().length > 1 ? (
                      ...
                      <div className="p-2">
                        ...
                        <div className="px-3 py-2 border-b border-border/50 mb-2">
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Browse by Category</span>
                        </div>
                        <div className="flex flex-wrap gap-2 p-1">
                          {["Subsidies", "Insurance", "Soil Health", "Loans", "Irrigation"].map((cat) => (
                            <button 
                              key={cat}
                              onClick={() => setSearchQuery(cat)}
                              className="px-3 py-1.5 rounded-full bg-secondary/80 text-[10px] font-bold text-secondary-foreground hover:bg-primary hover:text-primary-foreground transition-all border border-border/50"
                            >
                              {cat}
                            </button>
                          ))}
                    ...
                    <div className="bg-muted/30 p-3 border-t border-border flex justify-between items-center">
                      <span className="text-[10px] text-muted-foreground">Select a scheme to view details</span>
                      <kbd className="px-1.5 py-0.5 rounded border border-border bg-card text-[10px] font-bold text-muted-foreground shadow-sm">ESC</kbd>
                    </div>
                  </div>
                )}
              </div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 p-2 bg-card border-border shadow-2xl rounded-2xl">
                  ...
                  <DropdownMenuItem 
                    ...
                    className="p-2 rounded-xl focus:bg-primary/5 cursor-pointer flex items-center gap-2"
                  >
                    <User className="w-4 h-4 text-primary" />
                    <span className="text-sm font-medium">My Profile</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem 
                    onClick={() => setIsAppliedOpen(true)}
                    className="p-2 rounded-xl focus:bg-primary/5 cursor-pointer flex items-center gap-2"
                  >
                  ...
                  <DropdownMenuItem 
                    onClick={logout}
                    className="p-2 rounded-xl focus:bg-destructive/10 text-destructive cursor-pointer flex items-center gap-2"
                  >
      ...
      <AppliedSchemesModal 
        ...
        onClose={() => setIsAppliedOpen(false)} 
      />
    </>
  );
};
